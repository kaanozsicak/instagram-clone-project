...
export class AuthService {
  ...
  static async saveUserAdditionalInfo(uid, userData) {
    try {
      ...
      if (error.code === 'permission-denied' || 
        ...
        throw new Error('Firestore\'a yazma izniniz yok. Lütfen Firebase ayarlarınızı kontrol edin.');
      }
  ...
  static async updateUserProfile(uid, profileData, profileImage = null) {
    try {
      ...
      if (profileImage) {
        const imageRef = ref(storage, `profile_images/${uid}/${profileImage.name}`);
    ...
    try {
      ...
      switch(error.code) {
        case 'auth/account-exists-with-different-credential':
          throw new Error('Bu e-posta adresi farklı bir hesapla zaten kayıtlı');
        case 'auth/popup-blocked':
          throw new Error('Popup penceresi engellenmiş. Lütfen izin verin.');
    ...
    try {
      ...
      switch(error.code) {
        ...
        case 'auth/popup-closed-by-user':
          throw new Error('Giriş işlemi kullanıcı tarafından iptal edildi');
        default:
          throw new Error('Facebook girişi sırasında bir hata oluştu');
      }
    }
  }
  ...
  static async ensureCurrentUser() {
    return new Promise((resolve) => {
        const unsubscribe = this.onAuthStateChange((user) => {
            unsubscribe();
            if (user) {
                user.reload().then(() => resolve(user)).catch(() => resolve(null));
            } else {
                resolve(null);
            }
        });
    ...
    try {
      ...
      try {
        ...
        if (firestoreError.message.includes('Null value')) {
          console.warn('Firestore sorgu hatası, kullanıcı adı kontrolü atlandı');
          return true;
        }
  ...
  static async followUser(targetUserId) {
    try {
      ...
      const currentUserFollowersRef = doc(firestore, 'followers', currentUser.uid);
      const targetUserFollowersRef = doc(firestore, 'followers', targetUserId);
